...
class AdaptiveSoftmax(nn.Module):
    def __init__(self, input_size, cutoffs, scale_down=4):
        ...
        for i in range(len(cutoffs)-1):
            seq = nn.Sequential(
                ...
                nn.Linear(input_size // scale_down,
                          cutoffs[i+1] - cutoffs[i], False)
            )
    ...
    def log_prob(self, inp):
        ...
        n = inp.size(0)
        prob = inp.new_zeros(n, self.cutoffs[-1])
...
class AdaptiveLoss(nn.Module):
    def __init__(self, cutoffs):
        ...
        for i in self.cutoffs:
            self.criterions.append(nn.CrossEntropyLoss(reduction='sum'))
    ...
    def forward(self, inp, target):
        ...
        for i in range(len(inp)):
            if inp[i] is not None:
                assert(target[i].min() >=
                       0 and target[i].max() <= inp[i].size(1))
