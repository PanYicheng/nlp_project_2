...
class BeamRerankDecoder():
    def __init__(self, predictor, scorers, coefs,
        ...
        if self.learn:
            ...
            # initialize using given coefs
            model.coefs.weight.data = torch.FloatTensor(
                self.coefs.reshape(1, -1))
            if ranking_loss:
    ...
    def decode(self, init_tokens, cont_tokens=None, itos=None, sample=True):
        ...
        beam = self.predictor.top_k_next(
            beam, self.beam_size, temperature=self.temperature)[0]
        beam = list(
            filter(lambda c: c.cont_tokens[-1] != dictionary['<unk>'], beam))
        gold_cont_raw_scores = None
        best = None
        step = 2
        cont_latest_scores = log(0.34)
        while (((best is None)
                or (best.adjusted_score < max(map(lambda c: c.score, beam))))
               and (step < self.max_len)):
            if self.verbosity > 0:
                print('{:-^80}'.format('Begin current beam'))
                for c in beam:
                    print(' '.join([itos[i]
                                    for i in c.cont_tokens]), end=' | ')
                    print('lm score: {:f} | '.format(c.score), end='')
                    if hasattr(c, 'raw_scores'):
                        print('lex,rel,rep score: ', c.raw_scores, end=' | ')
                    if hasattr(c, 'adjusted_score'):
                        print('adjusted score: {:f}'.format(c.adjusted_score),
                              end=' | ')
                    print('')
                print('{:-^80}'.format('End current beam'))
            conts = self.predictor.top_k_next(
                beam,
                self.beam_size,
                temperature=self.temperature)
            if self.verbosity > 0:
                print('{:-^80}'.format('Begin expanded beam'))
                for cs in conts:
                    for c in cs:
                        print(' '.join([itos[i]
                                        for i in c.cont_tokens]), end=' | ')
                        print('lm score: {:f} | '.format(c.score), end='')
                        if hasattr(c, 'raw_scores'):
                            print('lex,rel,rep score: ',
                                  c.raw_scores, end=' | ')
                        if hasattr(c, 'adjusted_score'):
                            print('adjusted score: {:f}'.format(c.adjusted_score),
                                  end=' | ')
                        print('')
                print('{:-^80}'.format('End expanded beam'))
                if self.verbosity > 2:
                    input()
            ...
            # if learn, add the gold continuation tokens to
            # get its scores.
            ...
            candidates = sorted(
                candidates, key=lambda c: c.adjusted_score, reverse=True)
            # filter out candidates that generates forbidden tokens, like <unk>
            filtered_candidates = list(filter(
                lambda c: c.cont_tokens[-1] not in self.forbidden, candidates))
            if sample and len(filtered_candidates) > self.beam_size:
                # use random sampling using adjusted_score
                # add .item because adjusted_score is torch tensor
                p = np.asarray(
                    list(map(lambda c: c.adjusted_score.item(), filtered_candidates)))
                p = p - max(p)
                p = np.exp(p / 1.0)
                p /= p.sum()
                # replace=False means that item will only be chosed once.
                beam = np.random.choice(
                    filtered_candidates, size=self.beam_size, replace=False, p=p)
            else:
                beam = filtered_candidates[:self.beam_size]
            # check if some candidate satisfy the story length
            # then get the best one
            for candidate in filter(lambda c: c.cont_tokens.count(dictionary['</s>']) == 5 and c.cont_tokens[-1] in self.terms, candidates):
        ...
        if self.learn:
            ...
            truth_lm_scores = self.predictor.logprobs(
                [init_tokens + cont_tokens]).squeeze().cpu().data.numpy()
            truth_lm_score = sum(
                [truth_lm_scores[i+len(init_tokens)-1, cont_tokens[i]]
                 for i in range(len(cont_tokens))])
            # use the best candidate in the beam
            # lm_scores = torch.Tensor([truth_lm_score, beam[0].score])
            # training_pair = [gold_cont_raw_scores, beam[0].raw_scores]
            lm_scores = torch.Tensor([truth_lm_score, best.score])
            training_pair = [gold_cont_raw_scores, best.raw_scores]
            ...
            loss.backward()
            self.optimizer.step()
            self.model.coefs.weight.data = self.model.coefs.weight.data.clamp(
                min=0)
