...
class RNNPredictor():
    def __init__(self, model, vocab_size, use_cuda=True, asm=False,
        ...
        #     model = model.cuda()
        if device is None:
            assert(False, "device should be passed to RNNPredictor")
        else:
    ...
    def top_k_next(self, beam, k, temperature=None, mask=None, vc=None):
        ...
        source = torch.LongTensor(tokens).t().to(self.device)
        # if output the per word log prob
        if self.asm:
            self.model.full = True
        output, hidden = self.model(source, hidden)
        if self.asm:
            output = output.view(source.size(
                0), source.size(1), self.vocab_size)
        else:
            output = output.data

        # extract the last word prob distribution
        temp = output[-1, :, :]
        # TODO: if temp above is already a log prob, the following log softmax
        # may not be needed, this could be a bug, confirmed that
        # log softmax is a redundant operation
    ...
    def logprobs(self, seqs):
        ...
        source = torch.LongTensor(seqs).t().to(self.device)
        if self.asm:
            self.model.full = True
        output, hidden = self.model(source, hidden)
        if self.asm:
    ...
    def top_next(self, cand, term):
        ...
        # source = Variable(torch.LongTensor(tokens).t().cuda()) -->
        source = torch.LongTensor(tokens).t().to(device)
        if self.asm:
            self.model.full = True
        output, hidden = self.model(source, hidden)
        if self.asm:
