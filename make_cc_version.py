...
parser = argparse.ArgumentParser(
    'Split text data into context and continuation')
...
for filename in filenames:
    ...
    with open(os.path.join(args.data_dir, filename), 'r') as lines:
        if args.doc_level:
            ...
            for line in lines:
                sents = [_.strip() for _ in line.split(args.sent_sym)]
                context = (' %s ' % args.sent_sym).join(
                    sents[:args.len_context]) + ' ' + args.sent_sym
                continuation = (' %s ' % args.sent_sym).join(sents[args.len_context:(
                    args.len_context+args.len_continuation)]) + ' '+args.sent_sym
