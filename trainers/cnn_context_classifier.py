...
class CNNContextClassifier(nn.Module):
    ...
    def forward(self, context, endings, itos=None):
        ...
        context_pooled = maxpool(context_convol).view(
            batch_size, self.embedding_dim)

        maxpool_end = nn.MaxPool1d(end_seq_len)
        # why use F.relu here ?
        end_conv = F.relu(self.ending_conv(self.embed_seq(end)))
        end_pooled = maxpool_end(end_conv).view(
            end_batch_size, self.embedding_dim)

        if decode_mode:
            context_pooled = context_pooled.expand(
                end_batch_size, self.embedding_dim).contiguous()
